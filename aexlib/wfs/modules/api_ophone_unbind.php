...
function api_ophone_unbind($api_obj) {
	//尚不开通取消绑定转移余额
	...
	$sp_sql	= "select * from ez_wfs_db.sp_wfs_ophone_cancel_ophone_bind_beta($1, $2, $3, $4, $5) ";

	$pg_params = array(
						$api_obj->params['api_params']['bsn'], 
						$api_obj->params['api_params']['imei'],
						$api_obj->params['api_params']['pno'],
						$api_obj->params['api_params']['unpass'],
						$api_obj->params['api_params']['isshift']
						);
	$wfs_db = new api_pgsql_db($api_obj->config->wfs_db_config,$api_obj);
	$wfs_return = $wfs_db->exec_db_sp($sp_sql, $pg_params);
	//$api_obj->write_hint(sprintf("Run here %s",$wfs_return['p_return']));
