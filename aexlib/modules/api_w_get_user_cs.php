...
*/
function w_get_user_cs($api_obj){
	//存储过程为合成存储过程
	...
	$rdata = $billingdb->billing_get_user_cs($resaler);
	$record = array ();
	if(is_array($rdata)){
		foreach ( $rdata as $key => $val ) {
			$record [$key] ['user_cs_id'] = @$val ['CS_ID'];
			$record [$key] ['user_cs_name'] = @$val ['Name'];
		}
