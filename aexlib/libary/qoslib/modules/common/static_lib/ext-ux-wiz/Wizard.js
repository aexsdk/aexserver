...
this.showLoadMask(true, 'validating');
     ...
     */
    loadMaskConfig : {
        'default' : lang_tr.saving,
        'plswait' : lang_tr.plswait
    },
     ...
     */
    finishButtonText : lang_tr.Finish,
...
// -------- helper
    ...
    showLoadMask : function(show, type)
    {
        ...
        if (show) {
            ...
            this.loadMask.msg = this.loadMaskConfig[type] || type;
            this.loadMask.show();
        } else {
            if (this.loadMask) {
                this.loadMask.hide();
    ...
    send_request: function(config){
        this.showLoadMask(true,config.waitMsg || 'plswait');
        ownerModule = config.ownerModule;
        // submit to server
        Ext.Ajax.request({
            waitMsg: config.waitMsg,
            url: ownerModule.app.connection,
            ...
            success: function(o){
                ...
                if(!o.responseText || o.responseText == ''){
                	Ext.Msg.show({
                        ...
                        msg: 'Update error,Response is empty!',
                        manager: ownerModule.app.getDesktop().getManager()
                    });
                	return;
                }
                var x = Ext.decode(o.responseText); //按照json格式解码返回值
                if (x) {
                    if (x.success) {
                        if (config.success) 
                            config.success(this, x);
                        else {
                            Ext.Msg.show({
                                title: lang_tr.Hint,
                                buttons: Ext.Msg.OK,
