...
class kernal {
   ...
   protected function print_css($items){
      ...
      if(is_array($items) && count($items > 0)){
         foreach($items as $item){
            $resp .= $this->print_item_css($item);
         }
      }

      // is the param an object?
      else if(is_object($items)){
         $resp .= $this->print_item_css($items);
      }
      return $resp;
   } // end print_css()
   ...
   private function print_item_css($item){
      ...
      if(!$this->is_item_css_loaded($item)){
         ...
         if($item->type == 'library'){
            $resp .= $this->print_library_css($item->id);
         }

         // is the item a module?
         else if($item->type == 'module'){
            $resp .= $this->print_module_css($item->id);
         }
   ...
   private function print_library_css($library_id){
      ...
      if($dependencies){
         $resp .= $this->print_css($dependencies);
      }
      ...
      $library_dir = path_to_url($this->get_library_dir());

      foreach($files as $file){
         $resp .= "<link rel='stylesheet' type='text/css' href='".$library_dir.$file."' />\n";
      }
   ...
   private function print_module_css($module_id){
      ...
      $m = $this->module->get_by_id($module_id);
      // get any dependencies and print their link tags first
      $dependencies = $this->module->get_dependencies($m);
      if($dependencies){
         $resp .= $this->print_css($dependencies);
      }

      // get the css files of the module and print their link tags
      $files = $this->module->get_client_files($m, 'css');
      ...
      //$document_root = $this->get_document_root();
      if(isset($m->is_plugin))
      	$module_dir = path_to_url($this->get_plugin_dir());
      else
      	$module_dir = path_to_url($this->get_module_dir());

      foreach($files as $file){
         $resp .= "<link rel='stylesheet' type='text/css' href='".$module_dir.$file."' />\n";
      }
   ...
   private function print_module_javascript($module_id){
      ...
      $files = $this->module->get_client_files($m, 'javascript');

	//echo sprintf("/*files=%s*/",json_encode($files));
      if(!$files){
         print '';
         return false;
      }
   ...
   private function validate_module($module_id){
      ...
      //echo sprintf("/*member=%s,group=%s,module=%s*/\r\n",$member_id,$group_id,$module_id);
      if(!$this->is_group_allowed($group_id, $module_id)){
      	 //echo sprintf("/* module %s is not allow*/",$module_id);
         return false;
      }
	  //echo sprintf("/*===member=%s,group=%s,module=%s*/\r\n",$member_id,$group_id,$module_id);
      ...
      if($m->is_plugin)
      	...
      	$module_dir = $this->get_module_dir();
      // get any dependencies and validate them first
      $dependencies = isset($m) && isset($m->dependencies) ? $m->dependencies:null;//$this->module->get_dependencies($module_id);
      if($dependencies){
         $success = $this->validate($dependencies);
         if(!$success){
            return false;
      ...
      if(isset($m->locale->directory, $m->locale->languages)){
         $locale_dir = $m->locale->directory;
         $extension = $m->locale->extension;
         $ls = $m->locale->languages;
