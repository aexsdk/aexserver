...
function api_ophone_action($api_obj)
{
    ...
    $mpass_return = $api_obj->get_from_api($api_obj->config->wfs_api_url,$data);
    //{"n_return_value":"-352","v_account":null,"p_return":"-352"}
	if ($mpass_return > 0) {//在wfs中修改密码执行成功
	    ...
	    if(is_array($billing_return)){
				...
				$api_obj->push_return_xml(sprintf('<MPASS>%s</MPASS>',$billing_return['RePassword']));
	    }
	}else{
		$api_obj->return_code = $mpass_return;
				//$api_obj->return_code = $rdata['ReturnValue'];
	}
		//写返回的信息
