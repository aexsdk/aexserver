...
function api_report_b2c($api_obj) {
/*
		
action=report_b2c
bsn=985Y9CP3520693
imei=357586001876728
pno=13189709677
pin=13189709677
pass=123456
prefix=0086
...
*/
		...
		$order_id      = empty($api_obj->params['api_params']['orderno']) ? $api_obj->params['api_params']['5'] : $api_obj->params['api_params']['orderno'];
		$v_order_num = explode(':', $order_id);
		...
		case 'pv':
			...
			$pv_type = 'b2b';
			$v_order_num = 'NULL';
			$sp_sql = "select * from ez_marketing_db.sp_mlm_n_cmd_add_b2c_order($1,$2,$3,$4,$5,$6)";
			break;
		default:
			//直销通过商城订单报单
	...
	if(is_array($mlm_return)){
		if($api_obj->return_code > 0)
		{
			...
			//echo "存储过程返回成功，写入成功的参数和代码";
			 $api_obj->push_return_data('PV_B2C',$mlm_return['n_oder_pv']);
		     $api_obj->push_return_data('STAR',$mlm_return['n_star']);
