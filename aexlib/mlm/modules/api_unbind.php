...
*/
function get_message_callback($api_obj,$context,$msg){
	return sprintf($msg,$api_obj->params['api_params']['pno'],$api_obj->return_code);
}
...
function api_unbind($api_obj) {
	$sp_sql	= "select * from ez_marketing_db.sp_mlm_n_cmd_unbind($1,$2,$3,$4) ";
	...
	if(is_array($mlm_return)){
		...
		if($api_obj->return_code > 0)
		{
			...
			$api_obj->push_return_xml("<UNBIND-OK>%s</UNBIND-OK>",$api_obj->return_code);
