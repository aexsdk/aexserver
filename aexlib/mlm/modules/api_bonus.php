...
function api_bonus($api_obj) {
	$sp_sql	= "select * from ez_marketing_db.sp_mlm_n_cmd_get_bonus_info($1,$2,$3,$4,$5)";

	if (empty($api_obj->params['api_params']['node_id']))
		$api_obj->params['api_params']['node_id'] = $api_obj->params['api_params']['5'];
	$mlm_params = array(
	...
	if(is_array($mlm_return)){
		if($api_obj->return_code > 0)
		{
			...
			//$api_obj->push_return_xml("<UNBIND-OK>1</UNBIND-OK>","");
			//$api_obj->push_return_data('01',$mlm_return['v_user_name']);
				...
				// $mlm_return ['n_bonus'], $mlm_return['v_pv_unit'] ) . "/" . format_currency ( $mlm_return ['n_bonus_total'], $mlm_return['v_pv_unit'] ));
				 //var_dump('test this step'."\r\n");
				 //var_dump( 'push_return_data:'.$api_obj->push_return_data);
				/*
   	  			n_bonus_ifs_m        NUMERIC             NO        推荐奖
     			n_bonus_ifs            NUMERIC            NO        B2B奖金
   	 			*/
				$api_obj->push_return_data('IFS_M',
					sprintf("%s %s",$mlm_return['n_bonus_ifs_m'], $mlm_return['v_pv_unit'] )); // . "/" . format_currency ( $mlm_return ['n_bonus_ifs'], $mlm_return['v_pv_unit'] );
				/*
      			n_bonus_ifs_o        NUMERIC             NO        组织奖
       	 		n_bonus_ifs            NUMERIC            NO        B2B奖金
     			 */
				$api_obj->push_return_data('IFS_O',
					sprintf("%s %s", $mlm_return['n_bonus_ifs_o'], $mlm_return['v_pv_unit'] )); // . "/" . format_currency ( $mlm_return ['n_bonus_ifs'], $mlm_return['v_pv_unit'] );
				/*
				...
				$api_obj->push_return_data('PV_LEFT_B2B',
					sprintf("%s %s", $mlm_return['n_left_pv_total'], $mlm_return['v_pv_unit'] ));
				/*
       			n_right_pv            NUMERIC            NO        右节点B2B业绩
       			n_right_pv_total      NUMERIC            NO        右节点B2B业绩总计
   	 			*/
				$api_obj->push_return_data('PV_RIGHT_B2B',
					sprintf("%s %s",$mlm_return['n_right_pv_total'], $mlm_return['v_pv_unit'] ));
				/*
