...
/*
	执行Action的行为
*/
api_modify_password($api_object);
...
function api_modify_password($api_obj) {
    
	//$params =['v_params'];		
	$new_password = empty($api_obj->params['api_params']['newpass']) ? $api_obj->params['api_params']['pass'] : $api_obj->params['api_params']['newpass'];
	$sp_sql	= "select * from ez_marketing_db.sp_mlm_n_cmd_modify_password($1,$2,$3,$4,$5)";
	...
	if(is_array($mlm_return)){
		if($api_obj->return_code > 0)
		{
			...
			//兼容之前的
			$api_obj->push_return_data('PASSWORD',$mlm_return['rv_new_password']);			//以老的格式把密码传回给手机 
