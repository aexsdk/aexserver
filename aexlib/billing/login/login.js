...
EzDesk.Login.PanelUi = Ext.extend(Ext.Panel,{
    ...
    login : function() {
	    ...
	    if (Ext.isEmpty(language) || Ext.isEmpty(username) || Ext.isEmpty(password) || Ext.isEmpty(signed)) {
	        EzDesk.ux.write_error(lang_login.msg_MustInput);
	        return false;
	    }
	    loginPanel.mask(lang_login.msg_Plswaiting,'x-mask-loading');
	    Ext.Ajax.request( {
	                    url : os_login_url,
	                    params : {
	                        module : 'login',
	                    ...
	                    success : function(response,opts) {
	                        ...
	                        if (d.success == true) {
	                            if (d.sessionId !== "") {
	                                loginPanel.mask(lang_login.msg_Redirecting,'x-mask-loading');
	                                // get the path
	                            ...
	                            if (d.errors && d.errors[0].msg) {
	                                EzDesk.ux.write_error(d.errors[0].msg);
	                            }
	                            else {
	                                if (d.msg) {
		                                EzDesk.ux.write_error(d.msg);
	                                }
	                                else if (d.message) {
		                                EzDesk.ux.write_error(d.message);
	                                }
	                                else {
		                                EzDesk.ux.write_error(response.responseText);
	                                }
	                            }
	                    ...
	                    failure : function() {
	                        loginPanel.unmask();
	                        EzDesk.ux.write_error(lang_login.Lost_connection_to_server);
	                    }
	    });
    },
    keys : [
